...
`トップページ` → `機能説明` → `利用前の確認` → `分野選択` → `質問1〜7` → `困りごと選択` → `任意入力` → `完成画面`
...
flowchart TD
  ...
  CANDIDATES -- "1件以上" --> CONCERNS["困りごと選択"]
  CONCERNS --> MAIN_SELECTED{"主な困りごとを<br/>1件選択した？"}
  MAIN_SELECTED -- "いいえ" --> MAIN_WAIT["『次へ』が無効<br/>困りごと選択に留まる"]
  MAIN_WAIT --> CONCERNS
  MAIN_SELECTED -- "はい" --> OTHERS["その他の困りごとを<br/>0〜3件、任意で選択<br/>主な困りごとは候補外"]
...
| 困りごと選択   | 主な困りごとを1件選択                | 「次へ」が無効                           | その他を最大3件選択でき、任意入力へ進む |
...
- 困りごと選択の「前へ」は質問7へ、任意入力の「前へ」は困りごと選択へ戻ります。
...
| 6. 困りごと選択                                                                  | 7. 任意入力                                                                                           |
| -------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| <img src="screenshots/06-concerns.jpg" width="300" alt="主な困りごとの選択画面"> | <img src="screenshots/07-details.jpg" width="300" alt="氏名、メールアドレス、自由記述の任意入力画面"> |
| 主な困りごとを1件、その他を最大3件選択する。                                     | 紹介状へ掲載する情報を必要に応じて入力する。                                                          |

| 8. 完成画面                                                                      |
| -------------------------------------------------------------------------------- |
| <img src="screenshots/08-result.jpg" width="300" alt="説明書と紹介状の完成画面"> |
| 回答に対応する相談先を確認し、画像保存・印刷・回答編集などへ進む。               |
