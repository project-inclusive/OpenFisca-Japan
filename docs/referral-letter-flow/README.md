...
flowchart TD
  ...
  ANSWERED -- "はい・質問7" --> CANDIDATES{"相談候補が1件以上ある？<br/>緊急度が none 以外、または紹介先あり"}
...
| 質問7の完了          | `urgency !== none` または紹介先ありの回答が1件以上 | 困りごとなし画面へ進み、紹介状は作らない | 困りごと選択へ進む                      |
...
- 7回答すべてが `urgency: none` かつ紹介先なしの場合は、紹介状を生成せず「大きな困り事はなさそうです」画面へ分岐します。`urgency: none` でも紹介先が設定されている回答は困りごと候補になります。
