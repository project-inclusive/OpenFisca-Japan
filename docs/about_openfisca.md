...
`世帯1`が`太郎`(所得: 10000円), `花子`(所得: 20000円), `世帯2`が`Tom`(所得: 1000円)の例で考えます。
  - `対象世帯.members("所得", 対象期間)`のように世帯員ごとの変数を取得するメソッドの返り値は、`ndarray([10000, 20000, 1000])`と全世帯員の値が格納された1次元のndarrayになります。
  - `対象世帯.max("所得", 対象期間)`のように世帯ごとの変数を取得するメソッドの返り値は、`ndarray([20000, 1000])`と世帯ごとの値が格納された1次元のndarrayになります。
