...
class 障害基礎年金_最大(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        障害等級 = 対象人物("障害基礎年金_等級_最大", 対象期間)
        年金額 = np.select([障害等級 == 1, 障害等級 == 2],
            [一級の支給額, 二級の支給額],
            0)

        所得一覧 = 対象人物("所得", 対象期間)
        所得降順 = 対象人物.get_rank(対象人物.世帯, -所得一覧)
        # 便宜上最も所得の多い世帯員を生計配偶者とする
        生計維持者である = 所得降順 == 0

        子の加算額 = 生計維持者である * 対象人物.世帯("障害基礎年金の子の加算額_最大", 対象期間)
...
class 障害基礎年金_最小(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        子の加算額 = 生計維持者である * 対象人物.世帯("障害基礎年金の子の加算額_最小", 対象期間)
...
class 障害基礎年金_等級_最大(Variable):
    ...
    def formula(対象人物, 対象期間, _parameters):
        ...
        二級の知的障害条件 = (愛の手帳等級 == 愛の手帳等級パターン.一度) +\
