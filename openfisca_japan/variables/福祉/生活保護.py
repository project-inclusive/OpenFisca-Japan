...
class 母子世帯等に係る経過的加算(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        return 配偶者がいない[0] * int(母子世帯等に係る経過的加算表()[世帯人数区分][str(年齢区分)][居住級地区分])
...
class 教育扶助基準(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        学年 = 対象世帯.members("学年", 対象期間)
        支給額一覧 = np.select([(学年 >= 1) * (学年 <= 6), (学年 >= 7) * (学年 <= 9)],
                              [2600, 5100],
                              0)
...
class 冬季加算(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        世帯人数 = 対象世帯("世帯人数", 対象期間)
        世帯人数区分インデックス = np.select([世帯人数 < 10],
                                           # インデックスは0始まりなので調整
                                           [世帯人数 - 1],
                                           -1).astype(int)
...
class 冬季加算地域区分1(Variable):
    ...
    def formula(対象世帯, 対象期間, _parameters):
        ...
        居住都道府県インデックス = np.select([居住都道府県 == 都道府県 for 都道府県 in 地域区分表キー一覧],
