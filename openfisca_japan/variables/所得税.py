...
from openfisca_japan.variables.障害.精神障害者保健福祉手帳 import (
    精神障害者保健福祉手帳等級パターン,
)
...
def 配偶者控除額表():
    ...
    return np.genfromtxt(
        COUNTRY_DIR + "/assets/所得/配偶者控除額.csv",
        delimiter=",",
        skip_header=1,
        dtype="int64",
    )[np.newaxis, 1:]
...
def 配偶者特別控除額表():
    ...
    """
    return np.genfromtxt(
        COUNTRY_DIR + "/assets/所得/配偶者特別控除額.csv",
        delimiter=",",
        skip_header=1,
        dtype="int64",
    )[:, 1:]
...
def 老人控除対象配偶者_配偶者控除額表():
    ...
    """
    return np.genfromtxt(
        COUNTRY_DIR + "/assets/所得/配偶者控除額_老人控除対象配偶者.csv",
        delimiter=",",
        skip_header=1,
        dtype="int64",
    )[np.newaxis, 1:]
...
class 障害者控除(Variable):
    ...
    def formula(対象人物, 対象期間, _parameters):
        ...
        扶養に入っていない納税者である = np.logical_not(
            所得が最も高い世帯員である
        ) * np.logical_not(被扶養者である)
        # 被扶養者は控除を受けない（扶養者が代わりに控除を受けるため）
        return (
            所得が最も高い世帯員である * (被扶養者の合計控除額 + 控除対象額)
            + 扶養に入っていない納税者である * 控除対象額
        )
...
class 障害者控除対象額(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        特別障害者控除対象 = 対象人物("特別障害者控除対象", 対象期間) * np.logical_not(
            同居特別障害者控除対象
        )
...
class ひとり親控除(Variable):
    ...
    def formula_2020_01_01(対象人物, 対象期間, parameters):
        ...
        return (
            親である * ひとり親控除額 * 対象ひとり親 * (所得 < ひとり親控除_所得制限額)
        )
...
class 同一生計配偶者である(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        # 所得が高いほうが控除を受ける対象となる
        所得順位 = 対象人物.get_rank(
            対象人物.世帯, -所得, condition=対象人物.has_role(世帯.親)
        )
...
class 配偶者控除(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        所得一覧 = 対象人物("所得", 対象期間)
        所得降順 = 対象人物.get_rank(
            対象人物.世帯, -所得一覧, condition=対象人物.has_role(世帯.親)
        )
        ...
        控除対象者の配偶者の所得区分 = np.select(
            [控除対象者の配偶者の所得 <= 同一生計配偶者_所得制限額], [0], -1
        ).astype(
            int
        ...
        老人控除対象配偶者控除額 = 老人控除対象配偶者_配偶者控除額表()[
            控除対象者の配偶者の所得区分, 控除対象者の所得区分
        ]
        ...
        配偶者控除額 = (
            np.logical_not(配偶者が老人控除対象である) * 通常配偶者控除額
            + 配偶者が老人控除対象である * 老人控除対象配偶者控除額
        )
    ...
    def formula(対象人物, 対象期間, _parameters):
        ...
        控除対象者の配偶者の所得区分 = np.select(
            [
                ...
                * (控除対象者の配偶者の所得 <= 950000),
                (控除対象者の配偶者の所得 > 950000)
                * (控除対象者の配偶者の所得 <= 1000000),
                (控除対象者の配偶者の所得 > 1000000)
                * (控除対象者の配偶者の所得 <= 1050000),
                (控除対象者の配偶者の所得 > 1050000)
                * (控除対象者の配偶者の所得 <= 1100000),
                (控除対象者の配偶者の所得 > 1100000)
                * (控除対象者の配偶者の所得 <= 1150000),
                (控除対象者の配偶者の所得 > 1150000)
                * (控除対象者の配偶者の所得 <= 1200000),
                (控除対象者の配偶者の所得 > 1200000)
                * (控除対象者の配偶者の所得 <= 1250000),
                (控除対象者の配偶者の所得 > 1250000)
                * (控除対象者の配偶者の所得 <= 1300000),
                (控除対象者の配偶者の所得 > 1300000)
                * (控除対象者の配偶者の所得 <= 1330000),
            ],
            list(range(9)),
            -1,
        ).astype(int)
        ...
        return (
            ...
            * 配偶者がいる
            * 対象所得区分に該当する
            * 配偶者特別控除額表()[控除対象者の配偶者の所得区分, 控除対象者の所得区分]
        )
...
class 扶養控除(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):
        ...
        同居している老人扶養親族である = 老人扶養親族である * np.logical_not(
            介護施設入所中
        )
