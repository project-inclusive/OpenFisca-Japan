...
class 障害者控除(Variable):
    ...
    def formula(対象人物, 対象期間, _parameters):
        ...
        扶養に入っていない納税者である = np.logical_not(扶養者である) * np.logical_not(被扶養者である)
        # 被扶養者は控除を受けない（扶養者が代わりに控除を受けるため）
        return 扶養者である * (被扶養者の合計控除額 + 控除対象額) + 扶養に入っていない納税者である * 控除対象額
...
class 控除後世帯高所得(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        障害者控除一覧 = 対象世帯.members("障害者控除", 対象期間)
        障害者控除 = 対象世帯.sum(障害者控除一覧)
        ひとり親控除一覧 = 対象世帯.members("ひとり親控除", 対象期間)
