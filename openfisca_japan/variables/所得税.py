...
class 同一生計配偶者である(Variable):
    ...
    def formula(対象人物, 対象期間, parameters):        
        所得 = 対象人物("所得", 対象期間)
        # 所得が高いほうが控除を受ける対象となる
        親内の所得順位 = 対象人物.get_rank(対象人物.世帯, -所得, condition=対象人物.has_role(世帯.親))
        配偶者である = (親内の所得順位 == 1)  # 親のうち所得順位が低い方が配偶者
...
class 配偶者控除(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        所得降順 = 対象世帯.get_rank(対象世帯, - 所得一覧, condition=親である)
        納税者である = 所得降順 == 0
        納税者の配偶者である = 所得降順 == 1
        納税者の所得 = 対象世帯.sum(所得一覧 * 納税者である)
        ...
        納税者の所得区分 = np.select(
            [納税者の所得 <= 9000000, 
             ...
             (納税者の所得 > 9500000) * (納税者の所得 <= 10000000)],  # 複数世帯のarrayのためand, orの代わりに *. +
            [0, 1, 2],
            -1).astype(int)  # intにできるようデフォルトをNoneではなく-1
...
class 配偶者特別控除(Variable):
    ...
    def formula(対象世帯, 対象期間, parameters):
        ...
        納税者の配偶者の所得区分 = np.select(
            [(納税者の配偶者の所得 > 480000) * (納税者の配偶者の所得 <= 950000),
             (納税者の配偶者の所得 > 950000) * (納税者の配偶者の所得 <= 1000000),
             (納税者の配偶者の所得 > 1000000) * (納税者の配偶者の所得 <= 1050000),
             (納税者の配偶者の所得 > 1050000) * (納税者の配偶者の所得 <= 1100000),
             (納税者の配偶者の所得 > 1100000) * (納税者の配偶者の所得 <= 1150000),
             (納税者の配偶者の所得 > 1150000) * (納税者の配偶者の所得 <= 1200000),
        ...
        return 配偶者がいる * 対象所得区分に該当する * 配偶者特別控除額表()[納税者の配偶者の所得区分, 納税者の所得区分]
