  ...
  useEffect(() => {
    if (household.世帯員[personName].愛の手帳等級) {
      aiItems.map((item, index) => {
        if (item[1] === household.世帯員[personName].愛の手帳等級[currentDate]) {
  ...
  useEffect(() => {
    if (household.世帯員[personName].療育手帳等級) {
      ryoikuItems.map((item, index) => {
        if (item[1] === household.世帯員[personName].療育手帳等級[currentDate]) {
  ...
  return (
    <>
      <FormControl>
        ...
        <Select value={selectedRyoikuItemIndex} className="form-select" onChange={onRyoikuChange} mb={3}>
          {ryoikuItems.map((item, index) => (
      ...
      <FormControl>
        ...
        <Select value={selectedAiItemIndex} className="form-select" onChange={onAiChange} mb={3}>
          {aiItems.map((item, index) => (
            <option value={index} key={index}>
              {item[0]}
            </option>
