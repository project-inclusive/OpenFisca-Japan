...
test('親: 仕事: 次の質問が「6か月以内に新しい仕事を始めましたか？」', () => {
  ...
  actor.send({ type: 'next' });
  expect(actor.getSnapshot().value).toBe('6か月以内に新しい仕事を始めましたか？');
});
...
test('親: 6か月以内に新しい仕事を始めましたか？: 値が設定されている', () => {
  ...
  expect(
    actor.getSnapshot().context['6か月以内に新しい仕事を始めましたか？']['親'][0]
  ).toEqual({ type: 'Boolean', selection: true });
});
...
test('親: 休職中ですか？: 次の質問が「休職中に給与の支払いがない状態ですか？」', () => {
  ...
  actor.send({ type: 'next' });
  expect(actor.getSnapshot().value).toBe('休職中に給与の支払いがない状態ですか？');
});
...
test('親: 休職中に給与の支払いがない状態ですか？: 値が設定されている', () => {
  ...
  expect(
    actor.getSnapshot().context['休職中に給与の支払いがない状態ですか？']['親'][0]
  ).toEqual({ type: 'Boolean', selection: true });
});
...
test('親: 病気やけが、障害はありますか？: 次の質問が「業務によって病気やけがをしましたか？」', () => {
  ...
  actor.send({ type: 'next' });
  expect(actor.getSnapshot().value).toBe('業務によって病気やけがをしましたか？');
});
...
test('親: 在宅療養中（結核、または治療に3か月以上かかるもの）ですか？: 「病気がある」を選択せず「障害がある」を選択した場合次の質問が「身体障害者手帳を持っていますか？」', () => {
  ...
  actor.send({ type: 'next' });
  // 身体障害者手帳へスキップ（業務による病気・けがと入院等はスキップ）
  // 「障害がある」のみ選択では業務によって病気やけが→はスキップして身体障害者手帳へ
  // しかし在宅療養中は経由しないのでskipParentUntilで到達するためここは直接テスト
  expect(actor.getSnapshot().value).toBe('身体障害者手帳を持っていますか？');
});
...
test('親: 血液凝固因子異常症のうち、当てはまるものはどれですか？: 値が設定されている', () => {
  const actor = createActor(questionStateMachine);
  actor.start();
  skipParentUntil(actor, '血液凝固因子異常症のうち、当てはまるものはどれですか？');
...
test('親: 身体障害者手帳を持っていますか？: 次の質問が「精神障害者保健福祉手帳を持っていますか？」', () => {
  ...
  actor.send({ type: 'next' });
  expect(actor.getSnapshot().value).toBe('精神障害者保健福祉手帳を持っていますか？');
});
...
test('親: 精神障害者保健福祉手帳を持っていますか？: 値が設定されている', () => {
  ...
  expect(
    actor.getSnapshot().context['精神障害者保健福祉手帳を持っていますか？']['親'][0]
  ).toEqual({ type: 'Selection', selection: '上記以外／持っていない' });
});
...
test('親: 療育手帳、または愛の手帳を持っていますか？: 値が設定されている', () => {
  ...
  expect(
    actor.getSnapshot().context['療育手帳、または愛の手帳を持っていますか？']['親'][0]
  ).toEqual({ type: 'Selection', selection: '上記以外／持っていない' });
});
...
test('親: 療育手帳、または愛の手帳を持っていますか？: next->backで元の質問に戻る', () => {
  ...
  actor.send({ type: 'next' });
  actor.send({ type: 'back' });
  expect(actor.getSnapshot().value).toBe('療育手帳、または愛の手帳を持っていますか？');
  expect(
    actor.getSnapshot().context['療育手帳、または愛の手帳を持っていますか？']['親'][0]
  ).toEqual({ type: 'Selection', selection: '上記以外／持っていない' });
});
...
test('親: 放射線障害がありますか？: 値が設定されている', () => {
  ...
  expect(actor.getSnapshot().context['放射線障害がありますか？']['親'][0]).toEqual(
    { type: 'Selection', selection: 'いいえ' }
  );
});
...
test('親: 内部障害（内臓などのからだの内部の障害）がありますか？: 次の質問が「脳性まひ、または進行性筋萎縮症ですか？」', () => {
  ...
  actor.send({ type: 'next' });
  expect(actor.getSnapshot().value).toBe('脳性まひ、または進行性筋萎縮症ですか？');
});
...
test('親: 脳性まひ、または進行性筋萎縮症ですか？: 値が設定されている', () => {
  ...
  expect(
    actor.getSnapshot().context['脳性まひ、または進行性筋萎縮症ですか？']['親'][0]
  ).toEqual({ type: 'Boolean', selection: true });
});
